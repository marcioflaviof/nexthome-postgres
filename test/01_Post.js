...
describe("POST TESTS", async () => {
    ...
    it("POST Img in User", (done) => {
        chai.request(server)
            .post("/register/image/" + userId + "/0")
            .attach(
                "file",
                fs.readFileSync("/nexthome-postgres/tmp/testimg/House01.jpeg"),
                "House01.jpeg"
            )
            .end((err, res) => {
                expect(res.statusCode).to.equal(200);
    ...
    it("POST Img in House", (done) => {
        chai.request(server)
            .post("/register/image/0/" + houseId)
            .attach(
                "file",
                fs.readFileSync("/nexthome-postgres/tmp/testimg/House01.jpeg"),
                "House01.jpeg"
